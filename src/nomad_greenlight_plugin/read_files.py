...
def read_single_file(file_path, first_file_mark=None):
    ...
    new_columns = {col: col.lower().replace(" ", "_") for col in data_file_object.data.columns}
    data_file_object.data.rename(columns=new_columns, inplace=True)
    data_file_object.data = reset_dtype(data_file_object.data)
    data_file_object.units = {new_columns[k]: v for k, v in data_file_object.units.items()}
    if first_file_mark is not None:
        data_file_object.data.loc[0, 'file_mark'] = first_file_mark
    data_file_object.data['file_mark'] = data_file_object.data['file_mark'].ffill()
    time_stamp = pd.to_datetime(data_file_object.data['time_stamp']).values.astype(np.int64) // 1e9
    data_file_object.data['time'] = time_stamp.copy()
...
def read_multiple_files_and_combine(mainfile, root_name, split_char='_'):
    ...
    sorted_file_list = sorted(file_list, key=lambda x: itemgetter(-1)(x.split(split_char)))
...
def read_files(mainfile):
    ...
    if file_number is not None:
        # Load multiple files
        data_file_object = read_multiple_files_and_combine(mainfile, root_name, split_char=split_char)
    else:
